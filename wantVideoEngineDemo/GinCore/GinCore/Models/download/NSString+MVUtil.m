...
@implementation NSString (MVUtil)
...
+ (NSString*)getResourcePathWithUrl:(NSString*)url
{
    ...
    if (fileExtension.length > 0) {
        fileName = [NSString stringWithFormat:@"%u.%@", [[url description] hash], fileExtension];
    } else {
        fileName = [NSString stringWithFormat:@"%u", [[url description] hash]];
    }
