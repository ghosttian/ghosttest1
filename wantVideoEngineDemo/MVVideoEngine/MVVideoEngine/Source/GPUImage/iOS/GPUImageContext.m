...
static GPUImageContext *sharedMovieWriterContext = nil;

+ (GPUImageContext *)sharedMovieWriterContext
{
	if (!sharedMovieWriterContext) {
		sharedMovieWriterContext = [[[self class] alloc] init];
		[sharedMovieWriterContext useSharegroup:[[[GPUImageContext sharedImageProcessingContext] context] sharegroup]];
	}
    return sharedMovieWriterContext;
}

+ (void)releaseSharedMovieWriterContext
{
	sharedMovieWriterContext = nil;
}
