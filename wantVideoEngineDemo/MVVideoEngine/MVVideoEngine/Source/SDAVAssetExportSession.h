...
@property (nonatomic, strong) id pixelFormatType; //chose kCVPixelFormatType_32BGRA or kCVPixelFormatType_420YpCbCr8BiPlanarFullRange,default is kCVPixelFormatType_420YpCbCr8BiPlanarFullRange

@property (nonatomic, weak) id<SDAVAssetExportSessionDelegate> delegate;

@property (nonatomic) BOOL needFixTransform;
 /**
 ...
 * The dictionary’s keys are from <CoreVideo/CVPixelBuffer.h>.
 */
@property (nonatomic, copy) NSDictionary *sdAudioSettings;
...
/**
 *Starts the asynchronous execution of an each frame export session.
 *
 *This method starts an asynchronous export operation and returns immediately.
 *@param handler A block that is invoked when the last frame is complete or in the event of writing failure.
 */
...
- (void)exportSession:(SDAVAssetExportSession *)exportSession renderFrame:(CVPixelBufferRef)pixelBuffer withPresentationTime:(CMTime)presentationTime toBuffer:(CVPixelBufferRef)renderBuffer currentIndex:(NSInteger)index;

@optional

- (void)frameExportSession:(SDAVAssetExportSession *)exportSession originalFrame:(CVPixelBufferRef)pixelBuffer currentIndex:(NSInteger)index;
